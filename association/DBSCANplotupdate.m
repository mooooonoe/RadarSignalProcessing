...
function s_processing = signal(frame_number)
    ...
    for iidx = 1:k-1
        ...
        hold on;
        CenterPointPlot = scatter(velocityAxis(idx_col_center), rangeBin(idx_row_center), 30, 'wo', 'filled');
        text_str = sprintf('    idx %d', iidx);
        text(velocityAxis(idx_col_center), rangeBin(idx_row_center),  text_str, 'Color', 'white', 'FontSize', 10);
