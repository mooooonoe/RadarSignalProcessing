...
clc;
close all;
load('\\223.194.32.78\Digital_Lab\Personals\Subin_Moon\Radar\0_u_DopplerMapData\jejari\jejari_radarCube.mat');
load('\\223.194.32.78\Digital_Lab\Personals\Subin_Moon\Radar\0_u_DopplerMapData\jejari\jejari_adc_raw_data.mat');
...
% %수정-------------------------------------------------------------------
% % mti filter
% mti_target = ones(256,128,4);
% mti_target(:,indSel,:) = radar_data_pre_3dfft(:,indSel,:);
% mti_target = squeeze(mti_target(:,:,chanIdx));
% 
% 
% figure;
% imagesc(velocityAxis,rangeBin,10*log(abs(mti_target)));
% xlabel('Velocity (m/s)');
% ylabel('Range (m)');
% yticks(0:2:max(rangeBin));
% title('Range-Doppler Map');
...
%% Angle FFT
% AngleWin = taylorwin(4,5,-60);
% AngleBin=-60:40:60;
% for rangefftsize= 1:256
%     for dopplerfftsize=1:128
%         Angledata = squeeze(doppler(dopplerfftsize,:,rangefftsize));
%         Anglefft = fftshift(fft(Angledata'.*AngleWin,4));
%         Angular(dopplerfftsize,:,rangefftsize) = Anglefft;
%     end
% end
% Xpow1 = abs(Angular);
% Xpow2 = squeeze(sum(Xpow1,1)/size(Xpow1,1));
% figure;
% colorbar;
% imagesc(AngleBin,rangeBin,Xpow2);

%각 Rx antenna의 peak value값
% for Rxant = 1:4
%     doppler_Rx{Rxant} =  doppler(:,Rxant,:);
%     Rx_data_128x256{Rxant} = squeeze(doppler_Rx{Rxant});
%     mti_out{Rxant} = 10*log10(abs(Rx_data_128x256{Rxant}'));
%     mti_out{Rxant}(:,64)=mti_out{Rxant}(:,68);
%     mti_out{Rxant}(:,65)=mti_out{Rxant}(:,68);
%     mti_out{Rxant}(:,66)=mti_out{Rxant}(:,68);
%     [max_mti{Rxant}, max_idx{Rxant}] = max(mti_out{Rxant}(:));
%     [max_row_mti{Rxant}, max_col_mti{Rxant}] = ind2sub(size(doppler_Rx{Rxant}), max_idx{Rxant});
%     peakval{Rxant} = doppler_Rx{Rxant}(max_row_mti{Rxant}, max_col_mti{Rxant});
% end


% mti_out(:,64)=mti_out(:,68);
% mti_out(:,65)=mti_out(:,68);
% mti_out(:,66)=mti_out(:,68);
% [max_mti, max_idx] = max(mti_out(:));
% [max_row_mti, max_col_mti] = ind2sub(size(mti_out), max_idx);
% figure;
% imagesc(velocityAxis,rangeBin,mti_out);
% xlabel('Velocity (m/s)');
% ylabel('Range (m)');
% yticks(0:2:max(rangeBin));
% title('Range-Doppler Map');
