...
HistoryMap = zeros(Nframe, NSample, NChirp);

for i = 0:16
    dbscanMap = Mapping(frame_number);
    dbscanMap(dbscanMap ~= 0) = 1;
    HistoryMap(frame_number, :,:) = dbscanMap(:,:);
...
for k = 1:16
    DATA.data.vel{k} = HistoryMap(k,:,:);
end

save('HistoryMap.mat', 'DATA');


function dbscanMap = Mapping(frame_number)
