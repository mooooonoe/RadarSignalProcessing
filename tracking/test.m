...
%HistoryMap = zeros(10, 2);

while frame_number < 143
    %CenterPointPlot = scatter(NaN, NaN, 10, 'o', 'filled');
    % xlabel('Velocity (m/s)');
    % ylabel('Range (m)');
    % yticks(0:2:max(rangeBin));

    [R, C] = signal(frame_number);
    HistoryMap(frame_number-32, 1) = R;
    HistoryMap(frame_number-32, 2) = C;
    ylim([min(rangeBin), max(rangeBin)]);
    xlim([min(velocityAxis), max(velocityAxis)]);
...
%save('HistoryMap.mat', 'HistoryMap');
function [R, C] = signal(frame_number)
    ...
    while true
        ...
        if objectCnt_CA < (numrangeBins/3.8)
            break;
        end
    ...
    while true
        ...
        if objectCnt_OS < (numrangeBins/3.8)
            break;
        end
    ...
    disp(['curr Frame No.', num2str(frame_number),'    OS_K=', num2str(factor_OS), ', CA_K=', num2str(factor_CA)]);
    ...
    for iidx = 1:k-1
        ...
        R = rangeBin(idx_row_center);
        C = velocityAxis(idx_col_center);
        data{iidx, 2} = sprintf('%f', rangeBin(idx_row_center));
        data{iidx, 3} = sprintf('%f', velocityAxis(idx_col_center));
        
        hold on;
        %CenterPointPlot = scatter(velocityAxis(idx_col_center), rangeBin(idx_row_center), 30, 'o', 'filled');
        %ylim([min(rangeBin), max(rangeBin)]);
        %xlim([min(velocityAxis), max(velocityAxis)]);
        ...
        % text(velocityAxis(idx_col_center), rangeBin(idx_row_center),  text_str, 'Color', 'black', 'FontSize', 10);
    ...
    %% update
    %set(CenterPointPlot, 'XData', velocityAxis(idx_col_center), 'YData', rangeBin(idx_row_center));
    % ylim([min(rangeBin), max(rangeBin)]);
    % xlim([min(velocityAxis), max(velocityAxis)]);
    % 
