%% DBSCAN
function [clusterGrid] = DBSCAN(eps, MinPts, NSample, NChirp, detected_points_2D, db_doppler_mti)

maxValue = (max(detected_points_2D(:)))/3*2;
[row_2d, col_2d] = find(detected_points_2D > maxValue);
sz_data = size(row_2d);
size_data = sz_data(1);
