...
rangeProfileData_peak_i = zeros(128,4,256);
channelData_peak_i = zeros(4, 128);
two_d_input = zeros(128,4,256);

for chirpIdx = 1:128
    for chIdx = 1:4
        peaks = 0;
        peak_locs = 0;
        rangeProfileData_peak_i(chirpIdx, chIdx, :) = abs(radarCubeData_mti(chirpIdx, chIdx , :));
    end
end
...
for chirpIdx = 1:128
    for chIdx = 1:4
    fft_result = squeeze(radarCube2DFFT(chirpIdx, chIdx, :));
    [peaks, peak_locs] = findpeaks(abs(fft_result), 'MinPeakHeight', minPeakHeight);
    channelData_peak_i(chIdx, peak_locs) = peaks;
    ...
    Th_peak = 0.1;
        ...
        for i = 1:length(stemRange)
            if stemRange(i) ~= 0
                ...
                two_d_input(chirpIdx, chIdx, locs) = stemRange(locs);
            end
        end
...
for rangebin_size = 1:256
    for chIdx = 1:4
        win_dop = hann(128);
        DopData1 = squeeze(two_d_input(:, chIdx, rangebin_size)); %여기 radarCubeData_mti->radarCubeData_demo
