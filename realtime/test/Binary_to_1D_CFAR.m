...
for rangebin_size = 1:NSample
   for chIdx = 1:NChan
       DopData1 = squeeze(LVDS(:, chIdx, rangebin_size));  %여기 radarCubeData_mti->radarCubeData_demo
       DopData = fftshift(fft(DopData1 .* win_dop, NChirp));
       doppler(:, chIdx, rangebin_size) = DopData;
   end
end
doppler_cell = doppler;
